...
class LogRegModelTrainer:
    def __init__(self, dataset_name, root_dir):
        self.dataset_name = dataset_name
        self.data_dir = os.path.join(root_dir, 'data')
        self.model_dir = os.path.join(root_dir, 'models', 'num', dataset_name, 'logreg')
        ...
        # Specify the model parameters for Logistic Regression
        logreg_params = {
            'penalty': 'l2',
            'C': 1.0,
            'solver': 'lbfgs',
        ...
        self.config = NumModelConfig(model_type='logreg', dataset=dataset_name, model_params=logreg_params)
    ...
    def train_and_save_model(self, model_name, model_params, target_column):
        """Trains the Logistic Regression model and saves it to a file."""
        ...
        model = LogisticRegression(**model_params_copy)
    ...
    def train_models(self):
        """Main method to train and save Logistic Regression models for each number."""
...
def main():
    ...
    for dataset in datasets:
        trainer = LogRegModelTrainer(dataset_name=dataset, root_dir=root_dir)
