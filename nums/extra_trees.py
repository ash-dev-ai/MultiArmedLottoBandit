...
class ExtraTreesModelTrainer:
    def __init__(self, dataset_name, root_dir):
        self.dataset_name = dataset_name
        self.data_dir = os.path.join(root_dir, 'data')
        self.model_dir = os.path.join(root_dir, 'models', 'num', dataset_name, 'extra_trees')
        ...
        # Specify the model parameters for Extra Trees
        et_params = {
            'n_estimators': 100,
            'max_depth': None,
            'random_state': 42
        ...
        self.config = NumModelConfig(model_type='extra_trees', dataset=dataset_name, model_params=et_params)
    ...
    def train_and_save_model(self, model_name, model_params, target_column):
        """Trains the Extra Trees model and saves it to a file."""
        ...
        model = ExtraTreesClassifier(**model_params_copy)
    ...
    def train_models(self):
        """Main method to train and save Extra Trees models for each number."""
...
def main():
    ...
    for dataset in datasets:
        trainer = ExtraTreesModelTrainer(dataset_name=dataset, root_dir=root_dir)
