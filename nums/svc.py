...
class SVCModelTrainer:
    def __init__(self, dataset_name, root_dir):
        ...
        self.model_dir = os.path.join(root_dir, 'models', 'num', dataset_name, 'svc')
        ...
        # Specify the model parameters for SVC
        svc_params = {
            'C': 1.0,
            'kernel': 'rbf',
            'gamma': 'scale',
        ...
        self.config = NumModelConfig(model_type='svc', dataset=dataset_name, model_params=svc_params)
    ...
    def train_and_save_model(self, model_name, model_params, target_column):
        """Trains the SVC model and saves it to a file."""
    ...
    def train_models(self):
        """Main method to train and save SVC models for each number."""
...
def main():
    ...
    for dataset in datasets:
        trainer = SVCModelTrainer(dataset_name=dataset, root_dir=root_dir)
