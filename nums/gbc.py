...
class GBCModelTrainer:
    def __init__(self, dataset_name, root_dir):
        self.dataset_name = dataset_name
        self.data_dir = os.path.join(root_dir, 'data')
        self.model_dir = os.path.join(root_dir, 'models', 'num', dataset_name, 'gbc')
        ...
        # Specify the model parameters for GBC
        gbc_params = {
            'learning_rate': 0.01,
            'n_estimators': 100,
            'max_depth': 5,
        ...
        self.config = NumModelConfig(model_type='gbc', dataset=dataset_name, model_params=gbc_params)
    ...
    def train_and_save_model(self, model_name, model_params, target_column):
        """Trains the GBC model and saves it to a file."""
        ...
        model = GradientBoostingClassifier(**model_params_copy)
    ...
    def train_models(self):
        """Main method to train and save GBC models for each number."""
...
def main():
    ...
    for dataset in datasets:
        trainer = GBCModelTrainer(dataset_name=dataset, root_dir=root_dir)
