...
class ChuaCircuit:
    def __init__(self, a=15.6, b=28.0, m0=-1.143, m1=-0.714, dt=0.01, steps=10000):
        self.a = a
        self.b = b
        self.m0 = m0
        self.m1 = m1
    ...
    def generate_chaos_data(self):
        ...
        def chua_dynamics(x, y, z):
            hx = self.m1 * x + 0.5 * (self.m0 - self.m1) * (np.abs(x + 1) - np.abs(x - 1))
            dx = self.a * (y - x - hx)
            dy = x - y + z
            dz = -self.b * y
            return dx, dy, dz
...
def run_chua(return_predictions=False):
    ...
    chua = ChuaCircuit(steps=len(train_combined))
    train_combined = chua.transform_with_chaos(train_combined)
    val_combined = chua.transform_with_chaos(val_combined)
    test_combined = chua.transform_with_chaos(test_combined)
    
    chua = ChuaCircuit(steps=len(train_pb))
    train_pb = chua.transform_with_chaos(train_pb)
    val_pb = chua.transform_with_chaos(val_pb)
    test_pb = chua.transform_with_chaos(test_pb)
    
    chua = ChuaCircuit(steps=len(train_mb))
    train_mb = chua.transform_with_chaos(train_mb)
    val_mb = chua.transform_with_chaos(val_mb)
    test_mb = chua.transform_with_chaos(test_mb)
    ...
    # Save predictions
    PredictionSaver.save_predictions([pd.DataFrame(predictions_combined), pd.DataFrame(predictions_pb), pd.DataFrame(predictions_mb)], "chua")
