...
class LogisticMap:
    def __init__(self, r=3.8, x0=0.5, steps=10000):
        self.r = r
        self.x0 = x0
        self.steps = steps
...
def run_logistic(return_predictions=False):
    ...
    logistic = LogisticMap(steps=len(train_combined))
    train_combined = logistic.transform_with_chaos(train_combined)
    val_combined = logistic.transform_with_chaos(val_combined)
    test_combined = logistic.transform_with_chaos(test_combined)
    
    logistic = LogisticMap(steps=len(train_pb))
    train_pb = logistic.transform_with_chaos(train_pb)
    val_pb = logistic.transform_with_chaos(val_pb)
    test_pb = logistic.transform_with_chaos(test_pb)
    
    logistic = LogisticMap(steps=len(train_mb))
    train_mb = logistic.transform_with_chaos(train_mb)
    val_mb = logistic.transform_with_chaos(val_mb)
    test_mb = logistic.transform_with_chaos(test_mb)
    ...
    # Save predictions
    PredictionSaver.save_predictions([pd.DataFrame(predictions_combined), pd.DataFrame(predictions_pb), pd.DataFrame(predictions_mb)], "logistic")
    
    if return_predictions:
        return predictions_combined, predictions_pb, predictions_mb

if __name__ == "__main__":
    logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levellevel - %(message)s')
    run_logistic()
