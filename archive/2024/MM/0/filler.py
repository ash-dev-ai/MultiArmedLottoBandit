# -*- coding: utf-8 -*-
"""
Created on Mon Jan 15 14:45:23 2024
...
def generate_random_numbers():
    ...
    winning_numbers = random.sample(range(1, 71), 5)  # 5 unique numbers from 1 to 70
    powerball = random.randint(1, 25)  # Powerball number from 1 to 25

    # Ensure that the Powerball number is different from the winning numbers
    while powerball in winning_numbers:
        powerball = random.randint(1, 25)

    # Shuffle the winning numbers list to make it random
...
def preprocess_data(df, earliest_date):
    ...
    # Ensure that 'winning_numbers' is a list of integers
    ...
    # Create new column 'pb' for the Powerball number
    df['pb'] = df['winning_numbers'].apply(lambda x: x[-1] if len(x) > 0 else None)
