...
class CatBoostModelTrainer:
    def __init__(self, dataset_name, root_dir):
        ...
        self.model_dir = os.path.join(root_dir, 'models', 'num', dataset_name, 'catboost')
        ...
        # Specify the model parameters for CatBoost
        catboost_params = {
            'learning_rate': 0.01,
            'iterations': 500,
            'depth': 5,
        ...
        self.config = NumModelConfig(model_type='catboost', dataset=dataset_name, model_params=catboost_params)
    ...
    def train_and_save_model(self, model_name, model_params, target_column):
        ...
        model = CatBoostClassifier(**model_params_copy)
    ...
    def train_models(self):
        """Main method to train and save CatBoost models for each number."""
...
def main():
    ...
    for dataset in datasets:
        trainer = CatBoostModelTrainer(dataset_name=dataset, root_dir=root_dir)
