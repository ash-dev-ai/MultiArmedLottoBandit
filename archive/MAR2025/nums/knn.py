...
class KNNModelTrainer:
    def __init__(self, dataset_name, root_dir):
        ...
        self.model_dir = os.path.join(root_dir, 'models', 'num', dataset_name, 'knn')
        ...
        # Specify the model parameters for KNN
        knn_params = {
            'n_neighbors': 5,
            'weights': 'uniform',
            'algorithm': 'auto'
        ...
        self.config = NumModelConfig(model_type='knn', dataset=dataset_name, model_params=knn_params)
    ...
    def train_and_save_model(self, model_name, model_params, target_column):
        """Trains the KNN model and saves it to a file."""
        ...
        model = KNeighborsClassifier(**model_params_copy)
    ...
    def train_models(self):
        """Main method to train and save KNN models for each number."""
...
def main():
    ...
    for dataset in datasets:
        trainer = KNNModelTrainer(dataset_name=dataset, root_dir=root_dir)
