...
class LightGBMModelTrainer:
    def __init__(self, dataset_name, root_dir):
        ...
        self.model_dir = os.path.join(root_dir, 'models', 'num', dataset_name, 'lightgbm')
        ...
        # Specify the model parameters for LightGBM
        lgbm_params = {
            'learning_rate': 0.01,
            'n_estimators': 100,
            'max_depth': 5,
        ...
        self.config = NumModelConfig(model_type='lightgbm', dataset=dataset_name, model_params=lgbm_params)
    ...
    def train_and_save_model(self, model_name, model_params, target_column):
        """Trains the LightGBM model and saves it to a file."""
        ...
        model = LGBMClassifier(**model_params_copy)
    ...
    def train_models(self):
        """Main method to train and save LightGBM models for each number."""
...
def main():
    ...
    for dataset in datasets:
        trainer = LightGBMModelTrainer(dataset_name=dataset, root_dir=root_dir)
